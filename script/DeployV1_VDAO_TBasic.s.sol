...
contract DeployV1_VDAO_TBasic is Script {
    function run() external {
        // --- 1. CONFIGURATION ---
        address tokenAddr = vm.envAddress("TOKEN_ADDRESS"); // Must be the VulnerableMembershipToken address
        require(tokenAddr != address(0), "TOKEN_ADDRESS required");

        // Use the deployer's key
        vm.startBroadcast();

        // --- 2. VULNERABLE DAO DEPLOYMENT ---
        // Step 2a: Define the 120 member addresses array.
        // NOTE: Replace this short list with your full 120-member array from your file.
        ...
        console.log("V1_DAO_ADDR (VulnerableDAO):", daoAddr);

        // --- 3. TREASURYBASIC DEPLOYMENT (BOUND TO DAO) ---
        // Pass the DAO's address to the Treasury constructor.
        // This is the CRITICAL FIX: it ensures only the DAO can call executePayment().
        TreasuryBasic treasury = new TreasuryBasic(daoAddr);
        address treasuryAddr = address(treasury);
        console.log("V1_TREASURY_ADDR (TreasuryBasic):", treasuryAddr);

        vm.stopBroadcast();

        // --- IMPORTANT: Record these two addresses (V1_DAO_ADDR and V1_TREASURY_ADDR) ---
    }
}

...
import {TreasuryBasic} from "../src/TreasuryBasic.sol";
import "forge-std/Script.sol";

contract DeployVulnerableDAO is Script {
    // Hardcoded DAO members
    address[120] public daoMembers = [
    ...
    function run() external {
        ...
        // 1️⃣ Deploy Treasury (initial DAO address can be address(0))
        TreasuryBasic treasury = new TreasuryBasic(address(0));

        // 2️⃣ Deploy Vulnerable DAO
        VulnerableDAO dao = new VulnerableDAO(address(treasury));

        // 3️⃣ Update Treasury DAO address if you modify TreasuryBasic to allow it
        // treasury.setDao(address(dao));

        // 4️⃣ Optionally, distribute voting power to hardcoded members
        for (uint256 i = 0; i < daoMembers.length; i++) {
            // For vulnerable DAO, manually set votingPower (gas-inefficient)
            // dao.votingPower[daoMembers[i]] = 1e18; // or however you define voting weight
        }

        console.log("TreasuryBasic deployed at:", address(treasury));
        console.log("VulnerableDAO deployed at:", address(dao));
